...
# Category filter
unique_categories = all_data['Category'].dropna().unique()
selected_category = st.sidebar.selectbox("Filter by Category:", options=["All"] + list(unique_categories))
...
if search_query or selected_brand != "All" or selected_category != "All" or discount_only or shelf_query or (min_price != float(all_data['price'].astype(float).min()) or max_price != float(all_data['price'].astype(float).max())):
